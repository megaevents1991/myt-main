...
export type Event = {
  ...
  // Row creation time (Postgres default). The homepage "החדשים ביותר" row
  // sorts by it; older readers never selected it explicitly, so keep optional.
  created_at?: string;
  skip_flight?: boolean;
  ...
  // flight has no seats left for this event, so the package is sold out even
  // though its tickets say otherwise. Only ever set on events that came through
  // the events loader.
...
export type Coupon = {
   ...
   * per order. Percent coupons ignore it. Missing on pre-migration rows.
   */
