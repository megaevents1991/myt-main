...
import { countTickets, creditAccrued, isPaid, round2, wasSettledAtCutoff } from "@/lib/partner-commission";
...
type LedgerRow = Pick<PartnerCreditRedemption, "id" | "amount_usd" | "coupon_code" | "created_at">;
...
async function loadCredit(trackingCode: string): Promise<PartnerCredit> {
  ...
  const reservations = accrualStart
    ? allReservations.filter(
        (r) => (r.created_at ?? "") >= accrualStart || !wasSettledAtCutoff(r.billed_at),
      )
    : allReservations;
  ...
  // remainder.
  const codes = rawHistory.map((row) => (row.coupon_code ?? "").trim()).filter(Boolean);
  ...
  if (codes.length > 0) {
    const [usageResult, stateResult] = await Promise.all([
      ...
      supabase.from("coupons").select("code,times_used,max_uses").eq("partner_tracking_code", trackingCode),
    ]);
  ...
  const redeemedUsd = round2(settled.reduce((sum, r) => sum + Number(r.amount_usd ?? 0), 0));
...
export type ConvertResult = { ok: true; code: string; amountUsd: number } | { ok: false; error: string };
 ...
 */
export async function convertCreditToCoupon(options: ConvertOptions = {}): Promise<ConvertResult> {
  const session = await requirePartner();
 ...
 */
async function rollbackConversion(ledgerId: number, couponId: number, context: string): Promise<void> {
  ...
  if (ledgerResult.error || couponResult.error) {
    console.error(
      `CRITICAL convertCreditToCoupon: rollback failed (${context}) — ledger ${ledgerId} (${
        ledgerResult.error ? "FAILED: " + JSON.stringify(ledgerResult.error) : "ok"
      }), coupon ${couponId} (${
        couponResult.error ? "FAILED: " + JSON.stringify(couponResult.error) : "ok"
...
async function convertFor(
  ...
  if (credit.balanceUsd < MIN_CONVERT_USD) {
    return { ok: false, error: `צריך לפחות $${MIN_CONVERT_USD} בצבירה כדי להפיק קופון` };
  }
  ...
  if (ledgerError) {
    ...
    const { error: cleanupError } = await supabase.from("coupons").delete().eq("id", couponId);
    if (cleanupError) {
      console.error("convertCreditToCoupon: failed to clean up orphaned inactive coupon:", JSON.stringify(cleanupError));
    }
  ...
  if (round2(priorRedeemed + amountUsd) > available + 0.001) {
    ...
    return { ok: false, error: "הצבירה הזו הומרה הרגע. רעננו כדי לראות את היתרה." };
  }
  ...
  if (activateError) {
    ...
    return { ok: false, error: `הקופון ${finalCode} נוצר אך לא הופעל. פנו אלינו.` };
  }
