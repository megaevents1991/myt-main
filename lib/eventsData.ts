...
export async function getEvents(id?: number): Promise<{ events: Event[] }> {
  ...
  try {
    ...
    console.log(`[EventsData] Starting query - ID: ${id || 'all'}, futureDate: ${futureDate}`);
    ...
    console.log(`[EventsData] Query successful - Returned ${events?.length || 0} events in ${queryTime}ms`);
