...
assert.strictEqual(
  activityCategoryOf({ type: "music_event" }, MUSIC),
  "Concert",
);
assert.strictEqual(
  activityCategoryOf({ type: "sports_event" }, SPORT),
  "Sports",
);
assert.strictEqual(
  activityCategoryOf({ type: "sports_live_event_dynamic" }, SPORT),
  "Sports",
);
...
assert.strictEqual(
  activityCategoryOf({ type: "tx_event" }, SPORT, "artist"),
  "Sports",
);
...
assert.strictEqual(
  item.title,
  "גאנס אנד רוזס · ברלין · 2.10 · טיסה+מלון+כרטיס",
);
...
assert.strictEqual(
  playerLink.video_url,
  "",
  "player link dropped, row still valid",
);
