...
/** Meta's activity_category vocabulary, from event type then taxonomy root. */
export function activityCategoryOf(
  ...
  // tx_event carries either kind — fall back to the taxonomy root.
  const root = (taxonomy.categoryPath[0] ?? "").toLowerCase();
...
export function buildActivityItem(
  ...
  const activity_category = activityCategoryOf(event, taxonomy);
  ...
  const path = taxonomy.categoryPath;
  return {
    id: event.id,
    image_link,
    brand: FEED_BRAND,
    ...
    activity_sub_categories: path.length > 1 ? path[path.length - 1] : "",
    activity_date: eventDate,
    custom_label_0: (path[0] ?? "").toLowerCase(),
    custom_label_1: (path.length > 1 ? path[path.length - 1] : "").toLowerCase(),
  };
}
