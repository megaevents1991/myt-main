...
export async function GET(request: Request) {
  ...
  try {
    ...
    const calculateStats = (trackingData: any[], reservationsData: any[]) => {
      ...
      trackingData
        ...
        .forEach((t) => {
          ...
          if (
            !confirmedUsersMap.has(userId) ||
            numOfTickets > confirmedUsersMap.get(userId)
          ) {
            confirmedUsersMap.set(userId, numOfTickets);
          }
        });

      const totalConfirmedTickets = Array.from(
        confirmedUsersMap.values(),
      ).reduce((sum, count) => sum + count, 0);
      const totalPaidTickets = reservationsData.reduce((sum, reservation) => {
        const numOfTickets =
          reservation.event_order_info?.number_of_ticket || 0;
      ...
      return {
        ...
        eventsSelected: new Set(
          trackingData
            .filter((t) => t.stage === "EVENT_SELECTED")
            .map((t) => t.user_id),
        ).size,
        ticketsSelected: new Set(
          trackingData
            .filter((t) => t.stage === "TICKET_SELECTED")
            .map((t) => t.user_id),
        ).size,
        flightsSelected: new Set(
          trackingData
            .filter((t) => t.stage === "FLIGHT_SELECTED")
            .map((t) => t.user_id),
        ).size,
        hotelsSelected: new Set(
          trackingData
            .filter((t) => t.stage === "HOTEL_SELECTED")
            .map((t) => t.user_id),
        ).size,
    ...
    const statsThisMonth = calculateStats(
      trackingThisMonth,
    ...
    return NextResponse.json({
      trackingData,
      stats: { thisMonth: statsThisMonth, allTime: statsAllTime },
    });
  } catch (error) {
