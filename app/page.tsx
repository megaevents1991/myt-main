...
export default function Home() {
  ...
  const filteredOptions = events.filter(({ name, location }) => {
    const value = searchVale.toLowerCase().trim();
    return (
    ...
    <main>
      ...
      >
        ...
        >
          <Combobox.Target>
            <input
              className="w-full rounded p-2 text-main border border-secondary focus:border-main"
              dir="rtl"
              placeholder="חפש אירוע..."
              value={searchVale}
              onChange={(event) => {
                setSearchVale(event.currentTarget.value);
              ...
              onClick={() => searchVale.length > 1 && combobox.openDropdown()}
              onFocus={() => searchVale.length > 1 && combobox.openDropdown()}
